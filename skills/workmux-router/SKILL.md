...
- **Session Manager**: (다음 중 하나)
  - **cmux**: (모던한 세션 관리를 원하는 경우 권장)
  - **tmux**: (표준 세션 관리자)
...
- **세션 관리자 선택**: 시스템에 `cmux`가 설치되어 있거나 사용자가 선호하는 경우 `cmux`를 우선 사용하고, 그렇지 않으면 `tmux`를 사용한다.
...
2. 불가능한 경우:
   - 새 worktree 경로 출력
   - 세션명 출력
   - 다음 진입 방법을 간단히 안내 (예: `cmux attach <name>` 또는 `tmux a -t <name>`)
